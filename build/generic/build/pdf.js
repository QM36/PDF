...
/***/ (function(module, exports, __webpack_require__) {
  ...
  function getViewerConfiguration() {
    return {
      ...
      sidebar: {
        ...
        tabOutLine: document.getElementById('tabOutlineSwitch'),
        // attachmentsButton: document.getElementById('viewAttachments'),
        thumbnailView: document.getElementById('thumbnailView'),
        outlineView: document.getElementById('outlineView'),
        attachmentsView: document.getElementById('attachmentsView'),
        tabList: document.getElementById('sidebarTabList')
      },
      sidebarResizer: {
        outerContainer: document.getElementById('outerContainer'),
      ...
      findBar: {
        ...
        preArrow: document.getElementById("prev-button-image"),
        findNextButton: document.getElementById('findNext'),
        nextArrow: document.getElementById("next-button-image"),
  ...
  var DefaultExternalServices = {
    ...
    supportDocumentCopyAndPrint: true,
  };
  ...
  var PDFViewerApplication = {
    ...
    fitScale: function fitScale() {
      return this.pdfViewer._getSuitableScale(_ui_utils.DEFAULT_SCALE_VALUE);
    },
    get pagesCount() {
    ...
    get supportDocumentCopyAndPrint() {
      return this.externalServices.supportDocumentCopyAndPrint;
    },
          ...
          while (1) {
            switch (_context7.prev = _context7.next) {
              ...
              case 3:
                ...
                const password = this.pdfDocumentProperties['password'];
                if (password) {
    ...
    load: function load(pdfDocument) {
      ...
      pdfDocument.getDownloadInfo().then(function () {
        ...
        firstPagePromise.then(function () {
          ...
          if (window['fisrtScreenLoading']) {
            window['fisrtScreenLoading'].hide();
          }
        });
      });
  ...
  var validateFileURL;
  {
    var HOSTED_VIEWER_ORIGINS = ['null', 'http://docs.qq.com', 'https://docs.qq.com'];
  ...
  function webViewerInitialized() {
    ...
    // var pattern = /(?<=\?\w+\=)(.*)/;
    var file = (document.location.search.toString().split('?file=')[1]).split('&passwd=')[0];
    // if (index !== -1) {
    //   var file = document.location.search.toString().substring(index);
    // }
    // var file =  (document.location.search.toString().match(pattern) || ['empty'])[0];
    ...
    if (parameters) {
      if (parameters['passwd']) {
        var appProperty = PDFViewerApplication.pdfDocumentProperties;
        appProperty['password']=window.atob(parameters['passwd']);
      }
      if (parameters['copyable'] === 'false') {
        ...
        PDFViewerApplication.externalServices.supportDocumentCopyAndPrint = false;
      }
    }
    ...
    if (ua && (/(mobile)/i.test(ua))) {
      appConfig.bottomToolbar.setAttribute('style', 'display: none;');
    ...
    window.addEventListener('pdfToggleOutline', function(){
      PDFViewerApplication.pdfSidebar.toggle();
    });
    ...
    // document.getElementById('sidebarTabList').addEventListener('click', function(event){
  ...
  function webGuestureInit() {
    var container = document.getElementById('mainContainer');
    var wrapper = document.getElementById('outerContainer');
    container.style.transformOrigin = '0 0 0';
    var baseScrollTop = 0;
    var baseScrollLeft = 0;
    var ZOOM_MAX = 3.0;
    var ZOOM_MIN = 1.0;
    var PREVIEW_ZOOM_MIN = ZOOM_MIN - 0.2;
    var initScale = 1;
    var isZooming = false;
    var zoomScale = initScale;
    var touchX, touchY;

    function previewZoom(isDoubleClick = false) {
      ...
      container.style.transformOrigin = zoomScale <= 1 ? '50% 0 0' : '0 0 0';
      relocatePositionInWrapper(isDoubleClick);
      if (isDoubleClick) {
        wrapper.style.overflow = 'auto';
      }
    }

    function relocatePositionInWrapper(isDoubleClick) {
      if (baseScrollTop >= 0) {
        wrapper.scrollTop = (touchY + baseScrollTop * initScale) * (zoomScale / initScale) - touchY;
      }
      if (baseScrollLeft >= 0) {
        wrapper.scrollLeft = (touchX + baseScrollLeft * initScale) * (zoomScale / initScale) - touchX;
      }
    }

    function springAnimation(scale) {
      // container.style.webkitTransition = '300ms ease';
      ...
      if (scale <= 1) {
        container.style.transformOrigin = '50% 0 0';
      } else if (scale > 1) {
        container.style.transformOrigin = '0 0 0';
      }
    }
    new AlloyFinger(container, {
      multipointStart: function (evt) {
        ...
        baseScrollTop = parseInt(top / (Number(zoomScale)), 10);
        baseScrollLeft = parseInt(left / (Number(zoomScale)), 10);
        wrapper.style.overflow = 'hidden';
        try {
          touchX = (evt.touches[0].clientX + evt.touches[1].clientX) / 2;
          touchY = (evt.touches[0].clientY + evt.touches[1].clientY) / 2;
        } catch (err) {
          console.log(err);
        }
      },
      pinch: function(evt) {
        ...
        zoomScale = (initScale * Math.sqrt(evt.zoom)).toFixed(2);
        zoomScale = Math.min(Math.max(zoomScale, PREVIEW_ZOOM_MIN), ZOOM_MAX);
        previewZoom(false);
      },
      multipointEnd: function () {
        ...
        if (zoomScale < ZOOM_MIN) {
          zoomScale = ZOOM_MIN;
          springback = true;
        } else if (zoomScale > ZOOM_MAX) {
          zoomScale = ZOOM_MAX;
          springback = true;
        }
        if (springback) {
          springAnimation(zoomScale);
        }
        wrapper.style.overflow = 'auto';
      ...
      doubleTap: function (evt) {
        ...
        try {
            ...
            touchY = evt.changedTouches[0].clientY;
        } catch (err) {
            console.log(err);
        }
  ...
  function webViewerPageMode(evt) {
    ...
    PDFViewerApplication.pdfSidebar.switchView(view, false);
  }
  ...
  function webViewerKeyDown(evt) {
    ...
    if (cmd === 1 || cmd === 8) {
      switch (evt.keyCode) {
        case 83:
          if (PDFViewerApplication.supportDocumentCopyAndPrint) {
            PDFViewerApplication.download();
          }
          handled = true;
          break;
  ...
  function () {
    function PDFSidebar(_ref) {
      ...
      this.outlineTab = elements.tabOutLine;
    ...
    _createClass(PDFSidebar, [{
      ...
      value: function reset() {
        ...
        this.outlineTab.style.display = 'none';
      ...
      value: function _switchView(view) {
        ...
        switch (view) {
          ...
          case SidebarView.THUMBS:
              document.getElementById('tabThumbnail').classList.add('active');
          ...
          case SidebarView.OUTLINE:
            document.getElementById('tabOutlineSwitch').classList.add('active');
      ...
      value: function _addEventListeners() {
        ...
        this.tabList.addEventListener('click', function(event){
          ...
          switch (text) {
            case '缩略图':
              document.getElementById('tabOutlineSwitch').classList.remove('active');
              _this3.switchView(SidebarView.THUMBS);
              break;
            case '大纲':
                document.getElementById('tabThumbnail').classList.remove('active');
                _this3.switchView(SidebarView.OUTLINE);
              break;
            default:
        ...
        this.eventBus.on('outlineloaded', function (evt) {
          ...
          _this3.outlineTab.style.display = outlineCount ? 'block' : 'none';
          if (outlineCount) {
  ...
  function () {
    function PDFFindBar(options) {
      ...
      this.preArrow = options.preArrow || null;
      this.findNextButton = options.findNextButton || null;
      this.nextArrow = options.nextArrow || null;
      ...
      this.closeButton.addEventListener('mousedown', function(){
        _this.findField.value = '';
        _this.close();
      });
    ...
    _createClass(PDFFindBar, [{
      ...
      value: function updateResultsCount() {
        ...
        _this3.findNextButton.disabled=!total;
        _this3.findPreviousButton.disabled=!total;
        if (!total) {
          _this3.preArrow.style.opacity = 0.3;
          _this3.nextArrow.style.opacity = 0.3;
          _this3.findNextButton.style.removeProperty('background-color');
          _this3.findPreviousButton.style.removeProperty('background-color');
        } else {
          _this3.preArrow.style.opacity = 1.0;
          _this3.nextArrow.style.opacity = 1.0;
          _this3.findNextButton.style.backgroundColor = "#ffffff";
          _this3.findPreviousButton.style.backgroundColor = "#ffffff";
        }
        Promise.resolve(matchesCountMsg).then(function (msg) {
          _this3.findResultsCount.textContent = msg;
  
          _this3.findResultsCount.classList.toggle('hidden', !total && !_this3.findField.value);
  ...
  function () {
    function PDFThumbnailView(_ref) {
      ...
      pageLabel.className = 'pageTitle';
      pageLabel.innerHTML = this.id;
  ...
  function () {
    ...
    _createClass(BaseViewer, [{
      ...
      key: "_getSuitableScale",
      value: function _getSuitableScale(value) {
          var currentPage = this._pages[this._currentPageNumber - 1];
          if (!currentPage) {
            return;
          }
          ...
          switch (value) {
            ...
            default:
              console.error("".concat(this._name, "._getScale: \"").concat(value, "\" is an unknown zoom value."));
              return;
          }
          return scale;
        }
    ...
    _createClass(PDFPageView, [{
      ...
      value: function draw() {
        ...
        if (this.textLayerMode !== _ui_utils.TextLayerMode.DISABLE && this.textLayerFactory) {
          ...
          if (!PDFViewerApplication.supportDocumentCopyAndPrint) {
             textLayer.textLayerDiv.classList.add('textLayerDisable');
          }
        }
  ...
  function () {
    ...
    _createClass(Toolbar, [{
      ...
      value: function _bindListeners() {
        ...
        window.addEventListener('pdfFullScreen', function(){
          eventBus.dispatch('presentationmode', {
            source: self
          });
        });
      ...
      value: function _updateUIState() {
        ...
        // }
        //add by hongwu post message
        var params = {'pageNumber': pageNumber,
                      'pagesCount': pagesCount};
        var event = new CustomEvent('pdfPageUpdate', {detail: params});
  ...
  GenericExternalServices.createL10n = function (_ref) {
    var _ref$locale = _ref.locale,
        locale = 'zh-CN'; //_ref$locale === void 0 ? 'en-US' : _ref$locale;
  ...
  document.webL10n = function (window, document, undefined) {
    ...
    function loadLocale(lang, callback) {
      ...
      for (var i = 0; i < langCount; i++) {
        ...
        resource.load(langLinks[i].href, onResourceLoaded);
      }
    }
